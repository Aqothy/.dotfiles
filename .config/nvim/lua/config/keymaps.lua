...
map("x", "<s-j>", ":<C-u>execute \"'<,'>move '>+\" . v:count1<cr>gv=gv", { desc = "Move Down" })
map("x", "<s-k>", ":<C-u>execute \"'<,'>move '<-\" . (v:count1 + 1)<cr>gv=gv", { desc = "Move Up" })
map("x", "<bs>", [["_d]], { desc = "Delete without yanking" })

map({ "n", "x", "o" }, ";", rep.semicolon, { expr = true, desc = "Repeat next move" })
map({ "n", "x", "o" }, ",", rep.comma, { expr = true, desc = "Repeat previous move" })
map({ "n", "x", "o" }, "f", rep.builtin_f_expr, { expr = true, desc = "Repeat f/t forward" })
map({ "n", "x", "o" }, "F", rep.builtin_F_expr, { expr = true, desc = "Repeat F/T forward" })
map({ "n", "x", "o" }, "t", rep.builtin_t_expr, { expr = true, desc = "Repeat t/T forward" })
map({ "n", "x", "o" }, "T", rep.builtin_T_expr, { expr = true, desc = "Repeat T/T forward" })
...
map("i", "<C-CR>", "<c-o>o", { desc = "New line below" })
